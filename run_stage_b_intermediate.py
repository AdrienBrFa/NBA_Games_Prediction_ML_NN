"""
Main pipeline for NBA game prediction - Stage B1 Intermediate.

This variant uses a reduced feature set (40-60 features) compared to Stage B1 Full:
- Stage A1 historical features (20)
- Selected rolling metrics only (offensive/defensive/net rating, shooting %, key stats)
- Windows: last 5, last 10 games
- Optional seasonal reset for rolling windows

Configuration:
- FEATURE_SET = 'intermediate' (vs 'full' with 177 features)
...
# Stage configuration
STAGE_NAME = "stage_b_intermediate"
FEATURE_SET = "intermediate"  # 'intermediate' or 'full'
...
def main(threshold_metric="f1", model_version=1, embedding_dim=16):
    """
    Run the complete Stage B Intermediate pipeline.
    ...
    print(f"  Target Features: ~40-60 (vs ~177 in full)")
    print("="*80)
    ...
    # Step 2B: Engineer Stage B team statistics features (intermediate)
    print("\n" + "="*80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run Stage B1 Intermediate pipeline with configurable threshold optimization and model version")
    parser.add_argument(
        "--threshold_metric",
        type=str,
        default="f1",
